...
static void addItem(char* obj_id, size_t obj_id_sz)
{
	 ...
	 * then dump it.
	 */
        IMSG("Open Secure Storage for item: %s\n", obj_id);
...
static TEE_Result writeIntoMemory(const char* obj_id, size_t obj_id_sz, char* data, size_t data_sz)
{
	...
	obj_data_flag = TEE_DATA_FLAG_ACCESS_READ |		/* we can later read the oject */
        ...
        IMSG("Creating Persistent Object");
...
static TEE_Result generate_proof(void)
{
	...
	TEE_Result res;
        TEE_UUID uuid = TA_AUTH_SERVICE_UUID; 
	...
	if (!hash)
        ...
        writeIntoMemory(TA_AUTH_SERVICE_PROOF, strlen(TA_AUTH_SERVICE_PROOF), hash, hash_sz);
...
static TEE_Result readFromMemory(void)
{
	...
	char *data;
        ...
        service_id = TA_AUTH_SERVICE_STRING;
	...
	data = TEE_Malloc(object_info.dataSize, 0);
        ...
        /* Effectively retrieves the object data*/
        IMSG("Transfering data to object");
	res = TEE_ReadObjectData(object, data, object_info.dataSize, &read_bytes);
	if (res == TEE_SUCCESS)
                processItems(data);
	if (res != TEE_SUCCESS || read_bytes != object_info.dataSize)
...
static TEE_Result store_credentials(uint32_t param_types, TEE_Param params[4])
{
	...
	if (param_types != exp_param_types)
        ...
        /* Moves the paramenters contents to the local pointers*/
	...
	TEE_Free(data);
        ...
        itemKeys(keys);
        res = writeIntoMemory(TA_AUTH_SERVICE_STRING, strlen(TA_AUTH_SERVICE_STRING), keys, strlen(keys));
...
static TEE_Result load_credentials(uint32_t param_types, TEE_Param params[4])
{
	...
	if (param_types != exp_param_types)
        ...
        /* Moves the parameters values into the local pointers*/
	obj_id_sz = params[0].memref.size;
	obj_id = TEE_Malloc(obj_id_sz, 0);
	if (!obj_id)
		return TEE_ERROR_OUT_OF_MEMORY;
...
static TEE_Result get_attestation_proof(uint32_t param_types, TEE_Param params[4])
{
	...
	TEE_Result res;
        TEE_ObjectHandle object;
	TEE_ObjectInfo object_info;
        const char* obj_id = TA_AUTH_SERVICE_PROOF;
        uint32_t obj_id_sz = strlen(TA_AUTH_SERVICE_PROOF);
...
{
	switch (command) {
	case TA_AUTH_SERVICE_STORE_CREDENTIALS:
		return store_credentials(param_types, params);
	case TA_AUTH_SERVICE_LOAD_CREDENTIALS:
		return load_credentials(param_types, params);
	case TA_AUTH_SERVICE_DELETE_CREDENTIALS:
		return delete_credentials(param_types, params);
	case TA_AUTH_SERVICE_GET_PROOF:
